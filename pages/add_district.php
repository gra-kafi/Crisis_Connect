...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['divisions'])) {
    $selectedDivisions = $_POST['divisions'];

    // Convert array to a comma-separated string for SQL IN clause
    $division_ids = implode(",", array_map('intval', $selectedDivisions));

    // Query to get districts under those divisions
    $sql = "SELECT * FROM districts WHERE division_id IN ($division_ids)";
...
<body>
    ...
    <div class="container">
            ...
            <form action="add_upazilah.php" method="post">
                <?php
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<label = "chekbox-inline">';
                        echo '<input type="checkbox" name="districts[]" value="' . $row['district_id'] . '"> ' . $row['district_name'];
                        echo '</label>';
                    }
                } else {
