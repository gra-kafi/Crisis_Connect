...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['districts'])) {
    $selectedDistricts = $_POST['districts'];

    // Convert array to a comma-separated string for SQL IN clause
    $districts_ids = implode(",", array_map('intval', $selectedDistricts));

    // Query to get districts under those divisions
    $sql = "SELECT * FROM upazilas WHERE district_id IN ($districts_ids)";
...
<body>
    ...
    <div class="container">
            <div class="card">
            <h1>Select Upazilas</h1>
            <form action="add_union.php" method="post">
                <?php
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<label = "chekbox-inline">';
                        echo '<input type="checkbox" name="upazilas[]" value="' . $row['upazila_id'] . '"> ' . $row['upazila_name'];
                        echo '</label>';
                    }
                } else {
